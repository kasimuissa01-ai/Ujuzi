...
export default function InstaBioExercise({ step, onSuccess, canContinue }: Props) {
  ...
  const handleSelectOption = (word: string) => {
    ...
    if (!newFilled.includes(null)) {
      // For this specific exercise, accept Kiume/Kike in either order for blanks 0/1
      const isCorrect = 
  ...
  const Blank = ({ index }: { index: number }) => {
    ...
    return (
      <span 
        ...
        className={`inline-flex items-center justify-center min-w-[80px] h-8 px-3 mx-1 rounded-md text-sm font-bold align-middle transition-colors cursor-pointer ${
          isFilled 
            ? 'bg-[#1cb0f6] text-white shadow-sm'
            : isActive
              ? 'bg-[#37464f] border-2 border-[#1cb0f6] text-gray-400 border-dashed'
              : 'bg-[#202f36] border border-gray-600 text-gray-500'
        }`}
      >
      ...
      <div className="bg-slate-900 rounded-3xl border border-slate-700 overflow-hidden shadow-2xl flex flex-col mt-2">
        ...
        <div className="p-5 flex items-center gap-5 border-b border-slate-800 bg-slate-900">
          <div className="w-[72px] h-[72px] rounded-full bg-gradient-to-tr from-yellow-400 via-red-500 to-pink-500 p-[3px] shrink-0">
            <div className="w-full h-full bg-slate-900 border-[3px] border-slate-900 rounded-full overflow-hidden">
               <img src="https://fhtnqhkxpvrfzxrwwtso.supabase.co/storage/v1/object/public/Ujuzi_pkus/1778583556967-removebg-preview.png" alt="Profile" className="w-full h-full object-cover scale-110 translate-y-2 bg-[#131f24]" />
            </div>
          </div>
          <div className="flex flex-col">
            <div className="text-white font-bold text-[19px] flex items-center gap-1.5">
              Amina wa Viatu <Check className="w-[14px] h-[14px] text-white bg-blue-500 rounded-full p-[2px]" />
            </div>
            <div className="text-slate-400 text-[15px]">@amina_viatu</div>
          </div>
        </div>

        {/* Bio Section */}
        <div className="p-5 bg-slate-900 text-slate-100 text-[17px] leading-[1.8]">
             ...
             <AnimatePresence>
               {availableOptions.map((opt) => (
                 <motion.button
                   ...
                   className="px-5 py-3 bg-white text-slate-900 rounded-2xl font-bold text-[16px] shadow-sm border-b-[4px] border-slate-300 hover:bg-slate-50 transition-colors active:border-b-0 active:translate-y-[4px]"
                 >
                   {opt}
                 </motion.button>
               ))}
